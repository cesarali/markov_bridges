...
def conditional_music_experiment(number_of_epochs=3,sinusoidal=False)->CJBConfig:
    ...
    # data
    if sinusoidal:
        experiment_config.data = SinusoidalConfig(has_context_discrete=True) # CHANGE
    else:
...
if __name__=="__main__":
    ...
    if start:
        experiment_config = conditional_music_experiment(number_of_epochs=5,sinusoidal=True)
        experiment_config.trainer.debug = True # CHANGE
        experiment_config.trainer.device = "cpu"
        experiment_files = ExperimentFiles(experiment_name="cjb",
                                           experiment_type="music")    
